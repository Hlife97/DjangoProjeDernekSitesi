...
from product.models import Categori,Urun,Images,Comment
...
def index(request):
    sliderdata=(Urun.objects.filter(tip='kurs',status=True)|Urun.objects.filter(tip='etkinlik',status=True))
    duyuru=(Urun.objects.filter(tip='Duyuru',status=True)|Urun.objects.filter(tip='Haber',status=True))[:3]
    yaklasandata=Urun.objects.filter(tip='etkinlik',status=True).order_by('-update_at')[:2]
    ...
    context={
        ...
        'categori':categori,
        'yaklasan_etkinlik':yaklasandata,
        'duyuru':duyuru,
        'menu':menu,
...
def urun_detay(request,id,slug):
    ...
    comment=Comment.objects.filter(product_id=id,status=True).order_by('-update_at')
...
def content_detail(request,id,slug):
    ...
    products=Urun.objects.get(pk=id)
    images=Images.objects.filter(urun_id=id)
    comment=Comment.objects.filter(product_id=id,status=True).order_by('-update_at')[:5]
    benzer=Urun.objects.filter(Categori_id=products.Categori_id,status=True)[:3]
